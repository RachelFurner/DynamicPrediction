...
####################################################################################
####################################################################################
def ReadMITGCMfield(MITGCM_filename, split_ratio, exp_name, run_vars):
   ...
   inputs = np.zeros((0,4,da_T.shape[1],da_T.shape[2],da_T.shape[3])) 
   outputs = np.zeros((0,1,da_T.shape[1],da_T.shape[2],da_T.shape[3]))
   ...
   for time in range(0, min(data_end_index, da_T.shape[0]-1), 10):  
       ...
       input_temp = np.zeros((4,da_T.shape[1],da_T.shape[2],da_T.shape[3]))

       input_temp[0,:,:,:] = da_T[time,:,:,:]
       input_temp[1,:,:,:] = da_S[time,:,:,:]
       input_temp[2,:,:,:] = da_U[time,:,:,:]
       input_temp[3,:,:,:] = da_V[time,:,:,:]
       
       print(input_temp.shape)

       inputs = np.concatenate((inputs,input_temp),axis=0)
       print(inputs.shape)
